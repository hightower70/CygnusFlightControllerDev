...
static uint8_t l_expected_packet_length;
...
static void drvEthernetReleaseBuffer(uint8_t in_buffer_index);
...
void drvEthernetInit(void)
{
  ...
  if (WSAStartup(MAKEWORD(2,2),&wsa) != 0)
  {
	...
	interface_description.BufferRelease = drvEthernetReleaseBuffer;
	comAddInterface(&interface_description);
	...
	if( bind(l_server_socket, (struct sockaddr*)&server, sizeof(server)) == SOCKET_ERROR)
  {
	...
	while (!l_task_stop)
	{
		...
		if (l_socket != INVALID_SOCKET)
		{
			...
			if (l_receive_buffer_length > 0)
			{
				while (l_receive_buffer_pos < l_receive_buffer_length)
				{
					slipDecodeByte(&l_slip_decoder, l_receive_buffer[l_receive_buffer_pos++]);

					// store first byte of the receives packet as the packet length
					if (l_slip_decoder.TargetBufferPos == 1)
					{
						l_expected_packet_length = l_slip_decoder.TargetBuffer[0];
					}

					// check for packet end
					if (l_slip_decoder.Status != slip_ES_Idle && l_slip_decoder.TargetBufferPos == l_expected_packet_length)
					{
...
///////////////////////////////////////////////////////////////////////////////
/// @brief Mark buffer as free after processing its content
/// @param in_buffer_index Index of the buffer to set it to free
void drvEthernetReleaseBuffer(uint8_t in_buffer_index)
{
	l_packet_buffer_length[in_buffer_index] = 0;
	sysBinarySemaphoreUnlock(l_buffer_released_event);
}
