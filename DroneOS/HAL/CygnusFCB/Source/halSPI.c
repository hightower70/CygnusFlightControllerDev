...
///////////////////////////////////////////////////////////////////////////////
/// @brief Initialize SPI HAL layer
void drvSPIInit(void)
{
  ...
  l_spi.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_32;
...
void drvSPITransmit(uint8_t* in_buffer, uint16_t in_length)
{
	drvHAL_SetPinLow(SPI_CS_GPIO_Port, SPI_CS_Pin);

	HAL_SPI_Transmit_DMA(&l_spi, in_buffer, in_length);
}

void drvSPIWriteAndReadBlock(uint8_t* in_write_block, uint16_t in_write_block_length, uint8_t* out_read_block, uint16_t in_read_block_length)
{
	drvHAL_SetPinLow(SPI_CS_GPIO_Port, SPI_CS_Pin);
