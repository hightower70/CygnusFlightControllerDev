...
static void crcProcessMD5(crcMD5StateType* in_state, const uint8_t* in_data);
...
void crcUpdateMD5(crcMD5StateType* in_state, const uint8_t* in_data,
		uint32_t in_length)
{
	...
	if (offset)
	{
		...
		memcpy(in_state->InputBuffer + offset, p, copy);
		if (offset + copy < 64)
			return;

		p += copy;
		left -= copy;
		MD5Process(in_state, in_state->InputBuffer);
	}

	// Process full blocks.
	for (; left >= 64; p += 64, left -= 64)
		MD5Process(in_state, p);
...
void crcCloseMD5(crcMD5StateType* in_state, uint8_t out_hash[])
{
    ...
    MD5Update(in_state, pad, ((55 - (in_state->BitCount[0] >> 3)) & 63) + 1);

    // Append the length.
    MD5Update(in_state, data, 8);
    for (i = 0; i < 16; ++i)
			out_hash[i] = (uint8_t)(in_state->DigestBuffer[i >> 2] >> ((i & 3) << 3));
}

static void MD5Process(MD5StateType* in_state, const uint8_t* in_data)
{
    uint32_t a,b,c,d;
    uint32_t t;
    uint32_t xbuf[16];
    const uint32_t* X;
		...
		a = in_state->DigestBuffer[0];
		b = in_state->DigestBuffer[1],
		c = in_state->DigestBuffer[2];
		d = in_state->DigestBuffer[3];

    if (!((in_data - (const uint8_t *)0) & 3)) 
		{
			X = (const uint32_t*)in_data;
	  }
		else
		{
			/* not aligned */
			memcpy(xbuf, in_data, 64);
			X = xbuf;
    }
...
#undef SET
     ...
     /* Then perform the following additions. (That is increment each
        ...
        was started.) */
		in_state->DigestBuffer[0] += a;
    in_state->DigestBuffer[1] += b;
